...
class ModelEvaluation:
    ...
    def start_choosing_threshold(self, test_result):
    ...
    def start_evaluate(self):
        ...
        for qidx in tqdm(lis_ques_idx):
            ...
            x = np.array([self.data_builder.get_feature_vector(qidx, aidx) for aidx in lis_bm25_ranking])
            py = model_builder.predict_prob(x)
            g_truth = self.data_builder.get_relevant_aidx_of_ques(qidx)
            test_result.append((x, lis_bm25_ranking, py, g_truth))
        self.start_choosing_threshold(test_result)
