...
class SentBertDownstream:
    def __init__(self, pretrain_sent_bert):
        ...
        print('Initialize SentBERT in device ', self.device, ' from ', pretrain_sent_bert)
        self.model = SentenceTransformer(model_name_or_path=pretrain_sent_bert, device=self.device)
    ...
    def start_training(self, train_dataloader):
        ...
        n_save_step = 1000
        print('Save checkpoint in ', SENT_BERT_DOWNSTREAM_CHECKPOINT_PATH,
              'after ', n_save_step, 'step using: ', device)
